...
def plot_logs_gan(imgs_folder: str, run_name: str):
    ...
    plt.set_loglevel("WARNING")
    # Declares the path of the epochs and batch losses
    epochs_loss_csv = f"logs\{run_name}_training_log_epoch_gan.csv"
    batch_loss_csv = f"logs\{run_name}_training_log_batch_gan.csv"
    ...
    # Plots the validation MS-SSIM logs
    fig, ax = plt.subplots(figsize=(16, 8))
    ax.plot(x_epoch, val_ssim, color='r', linestyle='dashed', marker='o')
    fig.suptitle(f"{run_name} Validation MS-SSIM")
    ax.set_xticks(ticks_epoch)
    ax.set(xlabel="Epochs", ylabel="MS-SSIM")
