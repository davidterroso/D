...
def plot_logs_gan(imgs_folder: str, run_name: str, 
    ...
    if model_name == "GAN":
        ...
        with open(epochs_loss_csv) as file:
            ...
            for row in lines:
                x_epoch.append(row[0])
                val_epoch_loss.append(float(row[1]))
        ...
        ax1.plot(x_epoch, val_epoch_loss, 
                color='r', linestyle='dashed', 
                marker='o')
        ax1.set_title("Validation Loss/Epoch")
        ax1.set(xlabel="Epochs", ylabel="Loss")
        ax1.set_xticks(ticks_epoch) 

        ax2.plot(x_batch, train_batch_loss, 
                color = 'b', linestyle = 'dashed', 
                marker = 'o', alpha=0.03)
        ax2.set_title("Train Loss/Batch")
        ax2.set(xlabel="Batches", ylabel="Loss")
        ax2.set_xticks(ticks_batch)
